...
def arcgis_GeomtoPR(path,add,fields_add_input,projection,aut_field,proj_va,spec_aut,coder):
    # Vérification INPUT
    TRACE("************PARAMETERS************")
    ...
    TRACE('Ajout des champs à géocoder: {}'.format(add))
    ...
    if add is True:
        ...
        for field in fields_add:
            if field not in fields_names:
                ERREUR("Le champ {} n'est pas dans la table "
                       "=> Cochez l'option ajouter des champs"
                       " ou ajoutez manuellement le champ {}".format(field,field))
            else:
                TRACE("Le champ {} est bien dans la donnée".format(field))
                ...
                for idx2,coor in enumerate(pnt):
                    if idx2 > 0:
                        ERREUR("MULIPARTIE DANS LA DONNÉE => "
                               "problème avec l'entité {} dans la table (ou la selection)".format(idx))
                    for i in coor:
        ...
        for uidx,urow in enumerate(Ucursor):
            ...
            for f in fields_add:
                ...
                if f not in ("SHAPE@", aut_field):
                    ...
                    urow[idx_field[0]] = list_update[uidx][f]
...
if __name__ == '__main__':
    ...
    if Test is True:
        ...
        ADD          = arcpy.GetParameter(1) # Choix ajout de champ ou non (booléen facultatif)
    ...
    # Lancement du script
    arcgis_GeomtoPR(DATA,ADD,FIELDS_LIST,PROJ,FIELD_TARGET,PROJ_VA,SPEC_AUT,CODER)
